...
void my_exit_group(int status)
{
	...
	del_pid((pid_t) current->pid); // new add
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if(table[sysc].monitored == 2 || (table[sysc].monitored == 1 && check_pid_monitored(sysc, current->pid))){
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		return table[sysc].f(reg);
	}
	return table[sysc].f(reg);
	//printk("system call: %d is not being monitored by current pid: %d \n", sysc, current->pid);
//	return table[sysc].f(reg);
				...
				if (current_uid() == 0 || check_pid_from_list(current->pid, pid) == 0){
					switch (cmd) {
						case REQUEST_START_MONITORING:
							if (check_pid_monitored(syscall, (pid_t) pid) == 0){
								//add spin lock
								spin_lock(&pidlist_lock);
						...
						case REQUEST_STOP_MONITORING: 
							if (check_pid_monitored(syscall, (pid_t) pid) == 1){
								//add spin lock
								spin_lock(&pidlist_lock);
								if (del_pid_sysc((pid_t) pid, syscall) == 0) {
									// todo
									spin_unlock(&pidlist_lock);
									return 0;
								} else {
...
static int init_function(void) {
	...
	//save the original custom_syscall.
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	//save the original exit_group 
