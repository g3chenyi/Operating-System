...
struct frame {
	...
	unsigned long access_count; // for lru, to keep track access count
	int bit; // for clock, clock bit indicator, can only be 1 or 0.
	addr_t trace; // for opt, to record trace
	int fur;	// for opt, to record furtherest;
};
