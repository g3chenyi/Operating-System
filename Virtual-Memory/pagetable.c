...
char *find_physpage(addr_t vaddr, char type) {
        ...
        pgtbl_entry_t *sec_lvl_page_table = (pgtbl_entry_t *)(pgdir[idx].pde & PAGE_MASK);
        p = &sec_lvl_page_table[PGTBL_INDEX(vaddr)];

        // Check if p is valid or not, on swap or not, and handle appropriately

        if (!(p->frame & PG_VALID)){  // frame is invalid
                miss_count ++;
                
                int frame = allocate_frame(p);
                if (!(p->frame & PG_ONSWAP)){ 
                    // not on swap
                    // use allocated frame as first time
                    ...
                    // on swap
                    // use allocated frame as replacement of victim
                    p->frame = frame << PAGE_SHIFT; 
